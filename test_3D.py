...
def Inference(FLAGS):
    ...
    net = torch.nn.DataParallel(unet_3D(n_classes=num_classes, in_channels=1, is_train=False)).cuda()
    save_mode_path = os.path.join(
        snapshot_path, '{}_best_model.pth'.format(FLAGS.model))
    net.module.load_state_dict(torch.load(save_mode_path))
    #net=net.module
    ...
    with torch.no_grad():
        avg_metric = test_all_case(net, base_dir=FLAGS.root_path, method=FLAGS.model, test_list="test.txt", num_classes=num_classes,
                                   patch_size=(128, 128, 128), stride_xy=96, stride_z=96, test_save_path=test_save_path)
    return avg_metric
